(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["ArcheoLegend"] = factory();
	else
		root["ArcheoLegend"] = factory();
})(window, () => {
...
__webpack_require__.d(__webpack_exports__, {
  "appendRecordToLegend": () => (/* binding */ appendRecordToLegend),
  "appendRegionToLegend": () => (/* binding */ appendRegionToLegend),
  "clearLegend": () => (/* binding */ clearLegend),
  "generateLegendAttributeSection": () => (/* binding */ generateLegendAttributeSection),
  "initialize": () => (/* binding */ initialize),
  "initializeAttributionsSections": () => (/* reexport */ initializeAttributionsSections),
  "removeLayer": () => (/* binding */ removeLayer),
  "sortLayers": () => (/* binding */ sortLayers)
});

;// CONCATENATED MODULE: ./views/js-snippets/archeo-legend/utilities.js
        ...
        if (attributes[datasetKey][attributeKey].isOptgroup === true) {
          ...
          optionsKeys.forEach(optionKey => {
            ...
            if (optionKey in attributesTypes) {
              initializeAttributionsSections(selector, {
                temp: attributesTypes[optionKey]
              }, {});
              return;
            } else attributesDict[optionKey] = {
              name: attributeName,
              id: optionKey
            };
          });
          ...
          if (attributeKey in attributesTypes) {
            initializeAttributionsSections(selector, {
              temp: attributesTypes[attributeKey]
            }, {});
            return;
          } else attributesDict[attributeKey] = {
            name: attributeName,
            id: attributeKey
          };
        }
...
async function appendLayerToLegend(layer) {
  ...
  legendRecord = legendRecord.append($('<div/>', {
...
function updateLayersLegendVisibility() {
  if ($("#legend-layers .layer-legend:not(.hidden)").length === 0) $("#legend-layers").addClass('hidden');else $("#legend-layers").removeClass('hidden');
}
...
;// CONCATENATED MODULE: ./views/js-snippets/archeo-legend/events.js
...
function onLayerGradientChanged() {
  $('#legend-layers').on('layer-gradient-change', function (event, data) {
    var $wrapper = getLegendSectionEl(data.layerId);
    var layerSymbol = $wrapper.find('.legend-layer-symbol');
    layerSymbol.css('background', browser_default()(data.value).css());
    let layer = ArcheoMap.getLayer(data.layerId);
...
function onLayerVisibilityChanged() {
  $('#legend-layers').on('layer-visibility-change', function (event, data) {
    ...
    if (data.value === false || layerConfig.style.colorToggle === false) {
      $wrapper.addClass('hidden'); //$wrapper.css('display', 'none');
    } else {
      $wrapper.removeClass('hidden'); //$wrapper.css('display', 'block');
    }
...
;// CONCATENATED MODULE: ./views/js-snippets/archeo-legend/archeo-legend.js
...
async function clearLegend() {
  ...
  sections.addClass('hidden'); // Clear attributes sections content //

  let contents = sections.find('.attribute-section-content');
  contents.empty();
  valueCache.attributes = {}; // Clear regions sections //
...
async function appendRecordToLegend(attributeId, valueName, colorString) {
  if (!(attributeId in valueCache.attributes)) valueCache.attributes[attributeId] = {};
...
async function appendRegionToLegend(regionName, regionType) {
  if (!(regionType in valueCache.regions)) valueCache.regions[regionType] = {};
  ...
  if (!(regionName in regionsCache)) {
    ...
    if (regionName in regionsTypeLegend) {
      ...
      let regionStyle = regionsTypeLegend[regionName]; //var style = ArcheoMap.getLayerStyle(layer);
      ...
      if (ArcheoUtilities.isValid(regionStyle.pattern)) {
        ...
        ArcheoUtilities.changeColorOfBase64PNG(patternImage, [patternColor.r, patternColor.g, patternColor.b, patternColor.a || 1.0]).then(colorPatternImage => {
          var legendRecord = $('<div/>', {
            class: "region-legend",
...
function generateLegendAttributeSection(attributeInfo) {
  ...
  if (!isThereLegendSection(attributeInfo.id)) {
    ...
    }).appendTo(newSectionWrapper); // populate a section
...
function initialize(attributes, attributesTypes) {
  ...
  $legendContentWrapper.resizable({
    ...
    resize: function (event, ui) {
      ...
      let width = ui.size.width + 'px'; //let height = ui.size.height + 'px';
...
function removeLayer(layerId) {
  ...
  updateLayersLegendVisibility(); // Update attributes legend //
