...
async function initializeYearTexts(selector = '.years-text') {
	$(selector).each(function () {
		$(this).html( 
			ArcheoUtilities.getFormattedYear( $(this).text() ) 
		);
	});
}
...
function initializeYearInput($element, doInitialize = true, initialyTrigger = true) {
	...
	if(!$.fn.getDateNumberValue)
		$.fn.getDateNumberValue = function() {
	...
	if(doInitialize) {
		...
		$input.val( 
			ArcheoUtilities.getFirstNonEmptyValue( $input.attr('value'), $input.val() )
			);
	}
	...
	$input.change(function() {
		...
		if( properValue === '' )
			properValue = ArcheoUtilities.parseInput( $(this).attr('value') );
		else 
...
function initializeRangeSpinner($element, timelineMinMax, doInitialize = true, lang = 'en', setToMinMax = false) {
	...
	var $toWrapper = $element.find('.input-years-to');
	//var $singleWrapper = $element.find('.input-years');
	...
	var $to = $toWrapper.find('input');
	//var $single = $singleWrapper.find('input');
	...
	if(doInitialize) {
		...
		if(isFromModern) {
			...
			$from.attr('max', timelineMinMax[1]);
			//initialFromValue = ArcheoUtilities.getFirstNonEmptyValue( $from.attr('value'), "0" );
		...
		if(isToModern) {
			...
			$to.attr('max', timelineMinMax[1]);
			//initialToValue = ArcheoUtilities.getFirstNonEmptyValue( $to.attr('value'), "0" );
	...
	$from.change(function() {
        ...
        let properValue = ArcheoUtilities.parseInput( 
			ArcheoUtilities.getFirstNonEmptyValue( $(this).val(), $(this).attr('value') )
			);

		let toValue = ArcheoUtilities.parseInput( 
			ArcheoUtilities.getFirstNonEmptyValue( $to.val(), $to.attr('value') )
			);
	...
	$to.change(function() {
        ...
        let fromValue = ArcheoUtilities.parseInput( 
			ArcheoUtilities.getFirstNonEmptyValue( $from.val(), $from.attr('value') )
			);
...
function setOptionHTML($el, optionSettings, optionValue) {
	...
	if( ArcheoUtilities.isValid(optionSettings.selected) )    
...
function setSelect($el, options, addEmpty = false) {
	...
	for(var optionValue in options) {
		...
		if(optionSettings.isOptgroup) {
			...
			for(var optionValue2 in optionSettings.options) 
				setOptionHTML($optgroup, optionSettings.options[optionValue2], optionValue2);

			$el.append($optgroup);
		} else
			setOptionHTML($el, optionSettings, optionValue);
	}
}
...
const palettes = {
	...
	'gradient': [
		['#fff7f7', '#fffcf7', '#f7fff8', '#f7fffe', '#F6FAFE', '#fbf7ff', '#fff7fb'],
		['#d36464', '#d4a763', '#63d467', '#63d4c9', '#63A9D3', '#9c63d4', '#d4639d'],
		['#990b0b', '#99600b', '#0b990f', '#0b998b', '#0b4699', '#520b99', '#990b54']
	]
};


function initializeColorInput($element, options = {}, layerNumber = null, changeButtonColorEvent = null, eventsNames = ['move']) {
	if(changeButtonColorEvent === null)
		changeButtonColorEvent = function(event, color) {
	...
	var $spectrum = $element.spectrum({
		...
		hideAfterPaletteSelect: false, // leave it be
...
const defaultGradients = [
	['#fff7f7', '#d36464', '#990b0b'],
	['#fffcf7', '#d4a763', '#99600b'],
	['#f7fff8', '#63d467', '#0b990f'],
	['#f7fffe', '#63d4c9', '#0b998b'],
	['#F6FAFE', '#63A9D3', '#0b4699'],
	['#fbf7ff', '#9c63d4', '#520b99'],
	['#fff7fb', '#d4639d', '#990b54']
];


function initializeGradientInput($element, appendTo="body") {
	var gradientInputObj = new Grapick({
		el: $element[0], // It must be like that, otherwise it will throw jquery error
		direction: 'right',
		min: 0,
		max: 100
	});
...
function setGradientValue(gradientInputObj, gradient = null, layerNumber = null) {
	...
	let gradientArray;
	if(ArcheoUtilities.isString(gradient))
		gradientArray = ArcheoUtilities.gradientToArray(gradient);
	else
		gradientArray = gradient;

	if( !(ArcheoUtilities.isValid(gradientArray)) )
		gradientArray = defaultGradients[colorId];
	...
	for(var i = 0; i < gradientArray.length; ++i) {
		...
		if( i === 0 || i === gradientArray.length - 1 )
			handler.el.classList.add('grp-handler-fixed');
	}
...
function createPlotly(elId, data, title, options = {}) {
	return new Promise((resolution, rejection) => {
		...
		for(var i = 0; i < dataLength; ++i) {  
			...
			if(options.featureId === result.id)
				distinguishedFeature = result;
		...
		for(var key in tracesDic)
			traces.push( tracesDic[key] );
...
export {
	...
	initializeGradientInput,
	addSliderMark,
	initializeYearTexts,
	copyToClipboard,
	setSelectValueNoEvent,
	toggleCheckbox,
	toggleRadiobutton,
	setGradientValue,
	setSelectValue,
